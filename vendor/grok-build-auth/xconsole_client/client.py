...
class XConsoleAuthClient:
    ...
    def obtain_session_via_password(
        ...
        *,
        ...
        # Expose the last upstream failure to callers.  A Turnstile token is
        # single-use, so callers must solve a fresh challenge before retrying
        # terminal authentication failures.
        self._last_create_session_error = {
            "http_status": None,
            "grpc_status": None,
            "message": "",
        }
        ...
        for attempt in range(1, attempts + 1):
            for em in email_candidates:
                ...
                normalized_error = (grpc_msg or "").lower()
                if (
                    "invalid-credentials" in normalized_error
                    or "email or password" in normalized_error
                    or "turnstile" in normalized_error
                ):
                    # Retrying with this same one-time Turnstile token can only
                    # turn the useful first error into a misleading CF error.
                    return None
