...
class ProtocolOAuthClient:
    ...
    def authorize_existing_request(
        self,
        auth_url: str,
        *,
        expected_state: str = "",
        ...
        """Approve an externally-created PKCE request and return its code.

        Sub2API owns the verifier and token exchange.  This client only restores
        the post-signup xAI session, completes the cross-domain cookie-setter
        hop, submits consent, and returns the authorization code to Sub2API.
        """
        if session_cookies:
            self.load_cookies(session_cookies)
        sso = str((session_cookies or {}).get("sso") or "")
        if not sso:
            raise RuntimeError("OAuth authorization requires the post-signup SSO session")
        self._set_sso_cookie(sso)
        ...
        if expected_state and state != expected_state:
            raise RuntimeError("OAuth authorization state does not match Sub2API")
        if not all((state, redirect_uri, client_id, challenge)):
            raise RuntimeError("Sub2API OAuth URL is missing required PKCE parameters")

        consent_values = {
            "response_type": value("response_type", "code"),
            "client_id": client_id,
            "redirect_uri": redirect_uri,
            "scope": scope,
            "state": state,
            "code_challenge": challenge,
            "code_challenge_method": value("code_challenge_method", "S256"),
            "nonce": nonce,
        }
        consent_url = f"{ACCOUNTS_ORIGIN}/oauth2/consent?{urlencode(consent_values)}"

        # Prime the authorization server, then mint the domain cookies needed
        # by accounts.x.ai before opening the consent page.
        self._get(auth_url, allow_redirects=False)
        setter_result = self.create_cookie_setter_link(
            consent_url,
            error_url=f"{ACCOUNTS_ORIGIN}/sign-in",
            referer=f"{ACCOUNTS_ORIGIN}/sign-in?redirect=oauth2-provider",
        )
        if not setter_result.get("ok"):
            raise RuntimeError(
                f"CreateCookieSetterLink failed: {setter_result.get('error')}"
            )
        current = str(setter_result.get("cookie_setter_url") or "")
        for _ in range(8):
            if "code=" in current:
                return self._code_from_url(current, state)
            response = self._get(current, allow_redirects=False)
        ...
        page = self._get(current or consent_url, allow_redirects=False)
        location = page.headers.get("location") or page.headers.get("Location") or ""
        if location and "code=" in location:
            return self._code_from_url(urljoin(current or consent_url, location), state)
        html = page.text or ""
        action_sources = [html]
        for script_src in re.findall(r'<script[^>]+src=["\']([^"\']+)["\']', html, re.I):
            if len(action_sources) >= 16:
                break
            try:
                script_response = self._get(
                    urljoin(current or consent_url, script_src),
                    allow_redirects=True,
                )
                script_text = script_response.text or ""
                if "submitOAuth2Consent" in script_text:
                    action_sources.append(script_text)
            except Exception:
                continue
        action_text = "\n".join(action_sources)
        # Server Action IDs are deployment-specific. Only use IDs discovered
        # from the live consent page/bundles. A stale bundled ID can return
        # HTTP 200 with {"error":"Access denied"}, which is not a safe fallback.
        action_ids: List[str] = []
        for pattern in (
            r'createServerReference\)\("([a-f0-9]{40,64})"[^)]*submitOAuth2Consent',
            r'(?:next-action|serverReference|actionId)[^a-f0-9]+([a-f0-9]{40,64})',
            r'([a-f0-9]{40,64}).{0,600}submitOAuth2Consent',
            r'submitOAuth2Consent.{0,600}([a-f0-9]{40,64})',
        ):
            for match in re.finditer(pattern, action_text, re.I | re.S):
                candidate = match.group(1)
                if candidate not in action_ids:
                    action_ids.append(candidate)
        if not action_ids:
            raise RuntimeError(
                "xAI consent Server Action not found in the live page bundles"
            )
        ...
        body = _json.dumps(
            [{
                ...
                "codeChallengeMethod": value("code_challenge_method", "S256"),
                "nonce": nonce,
                "principalType": "User",
                "principalId": "",
                "referrer": "",
        ...
        headers = {
            ...
            "origin": ACCOUNTS_ORIGIN,
            "referer": current or consent_url,
            "sec-fetch-site": "same-origin",
            "sec-fetch-mode": "cors",
            "sec-fetch-dest": "empty",
        }
        submit_url = (current or consent_url).split("?", 1)[0]
        approved = None
        text = ""
        for action_id in action_ids:
            headers["next-action"] = action_id
            candidate_response = self._s.post(
                submit_url, headers=headers, data=body, timeout=45
            )
            candidate_text = candidate_response.text or ""
            if candidate_response.status_code == 404 and "server action" in candidate_text.lower():
                continue
            approved = candidate_response
            text = candidate_text
            location = candidate_response.headers.get("location") or candidate_response.headers.get("Location") or ""
            if location and "code=" in location:
                return self._code_from_url(urljoin(current or consent_url, location), state)
            match = re.search(r'"code"\s*:\s*"([^"]+)"', candidate_text)
            if not match:
                match = re.search(r'(?:[?&]|\\u0026)code=([A-Za-z0-9._~\-]+)', candidate_text)
            if match:
                return match.group(1)
            # A false-positive/stale action may be accepted by Next.js but
            # rejected by the application. Try the remaining discovered IDs.
            if "access denied" in candidate_text.lower():
                continue
            break
        if approved is None:
            raise RuntimeError(
                "xAI consent Server Action not found; authorization page action IDs changed"
            )
        raise RuntimeError(
            f"xAI consent did not return an authorization code: HTTP "
            f"{approved.status_code}: {text[:260]}"
        )
    ...
    def login(
        ...
        *,
        ...
        if sso_present:
            self._log("discovering live consent Server Action...")
            code = self.authorize_existing_request(
                auth_url,
                expected_state=state,
                session_cookies=session_cookies,
            )
            self._log("authorization code obtained via dynamic consent action")
